...
namespace Monitor
{
    ...
    Tracking::Tracking(std::shared_ptr<Monitor::Config>  ConfigInstance){
        ...
        // string str_img1=Monitor::Config::get<string> ( "dataset_first" );
    ...
    bool Tracking::InitFrame(string imgPath){
        ...
        // Monitor::Camera::Ptr camera ( new myslam::Camera );
        // 提取基准帧特征点即直线交点
        // 提取特征点
        // 计算描述子
        // 创建基准帧
    ...
    cv::Mat Tracking::GrabImageMonocular(const cv::Mat & im,double timestamp){
        ...
        // mpCurrentFrame = Monitor::Frame::createFrame();
    ...
    bool Tracking::AddFrame ( Frame::Ptr frame ,double time)//计算了每一帧的位姿
    {
    ...
    //     Mat K = ( Mat_<double> ( 3,3 ) << camera_->fx_, 0, camera_->fy_, 0, camera_->cx_, camera_->fy_, 0, 0, 1 );
